    ...
    public class PieMenu : ItemsControl
    {
        ...
        private void DrawMenu(ItemsControl items_control, int level, double angle, double sector, DrawingContext drawingContext)
        {
            ...
            double y = _size / 2.0;
                ...
                if (level == 1)
                {
                     inner_radius = InnerRadius + (Radius - InnerRadius + Gap) * level;
                    if (inner_radius < SectorGap) inner_radius = SectorGap;
                     outer_radius = Radius + (Radius - InnerRadius + Gap) * level*2.5;
                }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (this.RotateText) drawingContext.PushTransform(new RotateTransform((start_inner_angle + end_inner_angle) / 2.0 , center.X, center.Y));
